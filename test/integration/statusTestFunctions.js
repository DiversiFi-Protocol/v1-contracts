...
async function getAllReserves(reserveManager) {
  const indexToken = await reserveManager.getIndexToken()
  return Promise.all(
    targetAssetParams.map(async params => {
      scaledBalance = await reserveManager.getSpecificReserves(params.assetAddress)
      trueBalance = await indexToken.balanceOf(reserveManager.target)
...
async function testMintNoMigration(indexToken, mintAmount) {
  const reserveManager = await indexToken.getReserveManager()
  const targetAssetParams = await reserveManager.getTargetAssetParamsList()
  //expect tracked reserves to increase as expected with mint
  const compoundingMintFeeQ96 = await reserveManager.getCompoundingMintFeeQ96()
  const predictedDeposits = targetAssetParams.map(params => {
    return utils.predictDeposit(mintAmount, compoundingMintFeeQ96, params)
  })
  const startingReserves = await getAllReserves(reserveManager)
  const startingTotalReservesScaled = await reserveManager.getTotalReservesScaled()
  ...
  const endingTotalReservesScaled = await reserveManager.getTotalReservesScaled()
  const endingReserves = await getAllReserves(reserveManager)
...
async function testBurnNoMigration(indexToken, burnAmount) {
  ...
  const currentAssetParams = await reserveManager.getCurrentAssetParamsList()
  const burnFeeQ96 = await reserveManager.getBurnFeeQ96()
  ...
  const startingIndexTotalSupply = await indexToken.totalSupply()
  await reserveManager.burn(burnAmount, "0x")
...
async function testAdminControlsNoMigration(deployParams) {
  //setAdmin
  await expect(
    deployParams.reserveManager.connect(unpriviledged).setAdmin(unpriviledged)
  ).to.be.revertedWith("only_admin")
  await deployParams.reserveManager.setAdmin(unpriviledged)
  await deployParams.reserveManager.connect(unpriviledged).setAdmin(deployParams.admin)

  //setMintFeeQ96
  const startingMintFee = await deployParams.reserveManager.getMintFeeQ96()
  const startingCompoundingMintFee = await deployParams.reserveManager.getCompoundingMintFeeQ96()
  const mintFee2 = utils.decimalToFixed(0.69)
  await deployParams.reserveManager.setMintFeeQ96(mintFee2)
  expect(await reserveManager.getMintFeeQ96()).to.equal(mintFee2)
  expect(await reserveManager.getCompoundingMintFeeQ96()).to.equal(utils.calcCompoundingFeeRate(mintFee2))
  await deployParams.reserveManager.setMintFeeQ96(startingMintFee)
  expect(await reserveManager.getMintFeeQ96()).to.equal(startingMintFee)
  expect(await reserveManager.getCompoundingMintFeeQ96()).to.equal(startingCompoundingMintFee)

  //setBurnFeeQ96
  const startingBurnFee = await deployParams.reserveManager.getBurnFeeQ96()
  const burnFee2 = utils.decimalToFixed(0.69)
  await deployParams.reserveManager.setBurnFeeQ96(burnFee2)
  expect(await reserveManager.getBurnFeeQ96()).to.equal(burnFee2)
  await deployParams.reserveManager.setBurnFeeQ96(startingBurnFee)
  expect(await reserveManager.getBurnFeeQ96()).to.equal(startingBurnFee)

  //setMaxReserves
  const startingMaxReserves = await deployParams.reserveManager.getMaxReserves()
  const maxReserves2 = 42069n;
  await deployParams.reserveManager.setMaxReserves(maxReserves2)
  expect(
    await deployParams.reserveManager.getMaxReserves()
  ).to.equal(maxReserves2)
  await deployParams.reserveManager.setMaxReserves(startingMaxReserves)
  expect(
    await deployParams.reserveManager.getAllReserves()
  ).to.equal(startingMaxReserves)

  //setMaxReservesIncreaseRateQ96
  const startingMaxReservesIncreaseRateQ96 = await deployParams.reserveManager.getMaxReservesIncreaseRateQ96()
  const maxReservesIncreaseRateQ962 = 42069n;
  await deployParams.reserveManager.setMaxReservesIncreaseRateQ96(maxReservesIncreaseRateQ962)
  expect(
    await deployParams.reserveManager.getMaxReservesIncreaseRateQ96()
  ).to.equal(maxReservesIncreaseRateQ962)
  await deployParams.reserveManager.setMaxReservesIncreaseRateQ96(startingMaxReservesIncreaseRateQ96)
  expect(
    await deployParams.reserveManager.getAllReservesIncreaseRateQ96()
  ).to.equal(startingMaxReservesIncreaseRateQ96)

  //setMaxReservesIncreaseCooldown
  const startingMaxReservesIncreaseCooldown = await deployParams.reserveManager.getMaxReservesIncreaseCooldown()
  const maxReservesIncreaseRCooldown = 42069n;
  await deployParams.reserveManager.setMaxReservesIncreaseCooldown(maxReservesIncreaseRCooldown)
  expect(
    await deployParams.reserveManager.getMaxReservesIncreaseCooldown()
  ).to.equal(maxReservesIncreaseRCooldown)
  await deployParams.reserveManager.setMaxReservesIncreaseCooldown(startingMaxReservesIncreaseCooldown)
  expect(
    await deployParams.reserveManager.getAllReservesIncreaseCooldown()
  ).to.equal(startingMaxReservesIncreaseCooldown)

  //setIsMintEnabled
  expect(await deployParams.reserveManager.getIsMintEnabled()).to.equal(true)
  await deployParams.reserveManager.setIsMintEnabled(false)
  expect(await deployParams.reserveManager.getIsMintEnabled()).to.equal(false)
  await deployParams.reserveManager.setIsMintEnabled(true)
  expect(await deployParams.reserveManager.getIsMintEnabled()).to.equal(true)

  //increaseEqualizationBounty
  const bountyAmount = 42069n
  await deployParams.reserveManager.mint(bountyAmount)
  await deployParams.indexToken.burn(bountyAmount)
  const bountyBefore = await deployParams.reserveManager.getEqualizationBounty()
  await deployParams.reserveManager.increaseEqualizationBounty(bountyAmount)
  const bountyAfter = await deployParams.reserveManager.getEqualizationBounty()
  expect(bountyAfter - bountyBefore).to.equal(bountyAmount)
}

async function testMintMidMigration(deployParams, nextPool) {
  expect(deployParams.reserveManager).mint(42069n, "0x").to.be.revertedWith("liquidity pool is migrating")
...
async function testPreMigration(deployParams) {
  const reserveManager = deployParams.reserveManager
