...
describe("ReserveManager - Mint/Burn Functions", function () {
  ...
  describe("equalizeToTarget", function() {
    ...
    it("applies reserves tracking and token transfers correctly", async function() {
      ...
      expect(poolBalance1After).to.equal(poolBalance1Before + utils.scaleDecimals(equalizationVectorScaled[1], 18n, assetParams1.decimals) + 1n)
      expect(poolBalance2After).to.equal(poolBalance2Before + utils.scaleDecimals(equalizationVectorScaled[2], 18n, assetParams2.decimals) + 1n)
      ...
      expect(callerBalance1After).to.equal(callerBalance1Before - utils.scaleDecimals(equalizationVectorScaled[1], 18n, assetParams1.decimals) - 1n)
      expect(callerBalance2After).to.equal(callerBalance2Before - utils.scaleDecimals(equalizationVectorScaled[2], 18n, assetParams2.decimals) - 1n)
      ...
      expect(allocation0).to.be.closeTo(0, 0.000000001)
      expect(allocation1).to.be.closeTo(0.5, 0.000000001)
      expect(allocation2).to.be.closeTo(0.5, 0.000000001)
    })
